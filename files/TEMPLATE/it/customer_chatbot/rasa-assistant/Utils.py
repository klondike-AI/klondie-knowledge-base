...
def fine_conversazione(timer, tracker):
    ...
    if timer == True: # controlla se era scaduto il timer (cioè se timer è True)
        if message_number_operatore == "0": #controlla se l'operatore si era connesso
            message = "Ci scusiamo ma attualmente non c'è nessun operatore disponibile.  \nVerrai ricontattato all'indirizzo email fornito appena possibile."
        else:
            message = "Ci scusiamo ma l'operatore non è più disponibile, puoi riprovare più tardi."
    else:
        message = "La chat con il nostro operatore è terminata."
...
def exit_after_operator(tracker):
    ...
    # toglie dalla tabella global_variables le righe vecchie che risalgono ad almeno 30 giorni fa
    check_date = date.today() - timedelta(days=30)
    cursor.execute("DELETE FROM global_variables WHERE timestamp <= \"" + str(check_date) + "\"")
    conn.commit()
