...
while True:
    ...
    if 'Training Core model...' in all_file:
        ...
        if epochs != -1:
            ...
            cursor.execute("UPDATE rasa_faq_models SET status = \"Avanzamento training: " + str(percentuale) + "%\" WHERE id = " + str(model_index))
            conn.commit()
            if 'Epochs: 100%' in core_training_file:
                cursor.execute("UPDATE rasa_faq_models SET status = \"Generazione modello\" WHERE id = " + str(model_index))
                conn.commit()
                break
    # Se non sta allenando il Core: 1- sta allenando NLU; 2- sta ancora generando le triple
        ...
        # Generazione triple
            ...
            cursor.execute("UPDATE rasa_faq_models SET status = \"Elaborazione input\" WHERE id = " + str(model_index))
            conn.commit()
