...
class ActionExtractFilter(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        message = "Hello!\nI'm your digital assistant and I'm here to help you!\nHow can I help you?"
...
class ActionRetrieveTypes(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if tracker.get_slot("name_1") is not None: 
            if len(tracker.get_slot("name_1")) > 0 and "exit" not in str(tracker.get_slot("name_1")).lower():
                conn.close()
                conn_tables.close()
        ...
        if root not in ignored_verbs:
            message = "What do you want to " + root + "?\n"
        elif root == "have information":
            message = "What do you want to " + root + " about?\n"
        else:
            message = "What in particular?\n"
        ...
        message = message + "(Or type \"exit\" to get out)"
        dispatcher.utter_message(message.replace("\n", "  \n"))
...
class ActionRetrieveName(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if type(entity_type) == str:
            if "exit" in entity_type or set([entity_type]).intersection(set(types_set)) == set() or entity_type in entity_set: #entity_type not in types_set:
                entity_name = entity_type 
        else:
            if "exit" in entity_type or set(entity_type).intersection(set(types_set)) == set() or set(entity_type).issubset(set(entity_set)): #entity_type not in types_set:
                entity_name = entity_type
                
        if "exit" in entity_type or "exit" in entity_name:
            conn.close()
            conn_tables.close()
            return [SlotSet("found_name", "true"), SlotSet("type_1", "exit"), SlotSet("name_1", "exit")]
        if entity_name != []:
            conn.close()
            conn_tables.close()
        ...
        if root not in ignored_verbs:
            ...
            if type(entity_type) == str: # stringa inserita nel form
                ...
                for e in entity_type:
                    message = message + e + "/"
            message = message[:-1] + " do you want to " + root + "?\n"
        elif root == "have information":
            message = "About which "
            if type(entity_type) == str: # stringa inserita nel form
                message = message + entity_type + " "
                ...
                for e in entity_type:
                    message = message + e + "/"
            message = message[:-1] + " do you want to " + root + " about?\n"
        else:
            message = "Which "
            if type(entity_type) == str: # stringa inserita nel form
                message = message + entity_type + " "
                ...
                for e in entity_type:
                    message = message + e + "/"
            message = message[:-1] + " in particular?\n"
...
class ActionRetrieveFaq(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        message = ""
        if (tipi != [] and "exit" in tipi) or (entita != [] and "exit" in entita) or (tipi == [] and entities["entity_type"] == "exit") or (entita == [] and entities["entity"] == "exit") or entities["action"] == "exit":
            message = "Exit.\nHow can I help you?"
            dispatcher.utter_message(message.replace("\n", "  \n"))
        ...
        if len(faq_title) == 0:
            ...
            if len(faq_title) != 0:
                ...
                # se ne trova più di una
                    ...
                    message = "Which of the following do you mean?\n"
                    buttons = []
                    ct = 1
                    for f in faq_title:
                        faq_title = f[1]
                    ...
                    conn.close()
                    conn_tables.close()
                    buttons.append({"title": "none of the previous ones", "payload": str(ct)})
                    dispatcher.utter_message(message.replace("\n", "  \n"), buttons=buttons)
                ...
                if len(faq_title_1) != 0:
                    ...
                    if len(faq_title_1) == 1:
                        ...
                        for e in eval(faq_title2):
                            name.append(e['entity'])
                            type_name.append(e['entity_type'])
                        message = "Do you mean: " + faq_title_1[0][3] + "?"
                ...
                # se NON trova neanche quelle, si rimuovono solamente le ENTITA' (ed i relativi sinonimi)
                    ...
                    if len(similar_entities) == 1:
                        ...
                        for e in eval(faq_title2):
                            name.append(e['entity'])
                            type_name.append(e['entity_type'])
                        message = "Do you mean: " + similar_row[0][0] + "?"
                        buttons = [{"title": "yes", "payload": "yes"}, {"title": "no", "payload": "no"}]
                    ...
                    if len(entities1) != 0:
                        ...
                        if len(entities1) == 1:
                            ...
                            for e in eval(entities2):
                                name.append(e['entity'])
                                type_name.append(e['entity_type'])
                            message = "Do you mean: " + entities1[0][3] + "?"
                            buttons = [{"title": "yes", "payload": "yes"}, {"title": "no", "payload": "no"}]
        ...
        # se trova la tripla completa
            ...
            if more_rows == 0 and len(faq_title) == 1:
                ...
                conn.close()
                conn_tables.close()
                buttons.append({"title": "none of the previous one", "payload": str(ct)})
                dispatcher.utter_message(message.replace("\n", "  \n"), buttons=buttons)
                check_more_entities = 0
                slots_sets = slots_sets + update_global_variables(["more_rows", "check_more_entities"], [more_rows, check_more_entities])
                return slots_sets + [SlotSet("action_1", action_name), SlotSet("type_1", type_name), SlotSet("name_1", name), SlotSet("check_another_faq", "si"), SlotSet("assertion", None)]
        ...
        message = cursor_tables.fetchall()[0][0]
        message = eval("'''" + message.replace("\\", "\\\\").replace("\\U", "U") + "'''")
        dispatcher.utter_message(message.replace("\n", "  \n"))
...
class ActionExit(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        conn.close()
        conn_tables.close()
        return slots_sets + [SlotSet("action_1", None), SlotSet("name_1", None), SlotSet("type_1", None), SlotSet("assertion", None), SlotSet("found_name", "false"), Restarted()]
...
class TypeForm(FormAction):
    ...
    def validate_type_1(self, value: Text, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any], ) -> Dict[Text, Any]:
        ...
        if len(type_or_entity) != 0:
            ...
            dispatcher.utter_message("The answer cannot be empty. Repeat the entry.")
            return {"type_1": None}
...
class AssertionForm(FormAction):
    ...
    def validate_assertion(self, value: Text, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any], ) -> Dict[Text, Any]:
        ...
        if more_rows == 0:
            ...
            # SI
            if value in ["yes", "sure", "yes thank you", "of course", "yes sure", "yes of course", "ok", "okay"]:
                return {"assertion": "si"}

            # NO
            if value in ["no", "not", "wrong", "false"]:
                return {"assertion": "no"}

            # Non valido
            dispatcher.utter_message("Invalid answer. Answer with \"yes\" or \"no\".")
        ...
        # Se "more_rows" != 0, si estra l'indice della risposta corretta tra quelle precedentemente proposte all'utente
            ...
            try:
                ...
                dispatcher.utter_message("Invalid answer. Answer with the number corresponding to the chosen question.")
                return {"assertion": None}
...
class ActionCustomFallback(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if question.replace(" ", "") == "":
            message = "Sorry, I didn't understand, be more precise by specifying action and name.\nHow can I help you?"
            dispatcher.utter_message(message.replace("\n", "  \n"))
...
class ActionRetrieveNameOnlyAction(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if len(entity_only_action) > 0 and "exit" not in str(entity_only_action).lower():
            conn.close()
            conn_tables.close()
        ...
        # se è una parola out-topic ("exit" o altro) la si mette come entità e si gestisce in ActionRetrieveFaq
        if "exit" in entity_type or set([entity_type]).intersection(set(types_set)) == set() or set([entity_type]).intersection(set(entity_set)) != set():
            entity_name = entity_type 
            entity_only_action = entity_type
        if "exit" in entity_type or "exit" in entity_name:
            conn.close()
            conn_tables.close()
            entity_type_only_action = "exit"
            entity_only_action = "exit"
        ...
        message = ""
        global ignored_verbs
        if (action != "" and action != []) and action_only_action not in ignored_verbs:
            message = "Which " + entity_type + " do you want to " + action_only_action + "?\n"
        else:
            message = "Which " + entity_type + " in particular?\n"
        ...
        message = message + "(Or digit \"exit\" to get out)\n"
        dispatcher.utter_message(message.replace("\n", "  \n"))
                ...
                if str(e['entities']).lower() != "none":
                    for e1 in eval(e['entities']):
                        ...
                        if type_or_entity in sinonimi or type_or_entity_lemma in sinonimi:
                            ...
                            conn.close()
                            conn_tables.close()
                            slots_sets = slots_sets + update_global_variables(["entity_type_only_action", "entity_only_action"], [entity_type_only_action, entity_only_action])
                            return slots_sets
...
class ActionForm(FormAction):
    ...
    def validate_action_1(self, value: Text, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any], ) -> Dict[Text, Any]:
        ...
        if len(value) != 0:
            ...
            dispatcher.utter_message("The action cannot be empty. Enter the action.")
...
class ActionRetrieveActionOnlyAction(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        # se l'utente ha scritto "exit", salva la conversazione e azzera le variabili
        if (type(entita) == str and entita.lower() == "exit") or (type(entita) != str and "exit" in entita):
            message = "Exit.\nHow can I help you?"
            dispatcher.utter_message(message.replace("\n", "  \n"))
        ...
        message = "What do you want to do in particular?\n"
        actions_set_with_synonyms = set()
        global ignored_verbs
        for t in actions_set:
            if t != "" and t != "none" and t not in ignored_verbs and t not in actions_set_with_synonyms:
                message = message + "- " + t + "\n"
...
class ActionDateTime(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        orario = dt_string.split(" ")[1][:-3]
        ora = dt_string.split(" ")[1].split(":")[0]
        new_ora = ""
        if int(ora) > 12:
            new_ora = str(int(ora) - 12) + ":" + orario.split(":")[1] + " P.M."
        elif int(ora) < 12 and int(ora) > 0:
            new_ora = ora + " A.M."
        elif int(ora) == 12:
            new_ora = ora + " P.M."
        elif int(ora) == 0:
            new_ora = str(int(ora) - 12) + ":" + orario.split(":")[1] + " A.M."
        else:
            new_ora = str(orario)
        
        day_today = calendar.day_name[datetime.strptime(dt_string.split(" ")[0].replace("/", " "), '%d %m %Y').weekday()]
        
        message = "Today is " + day_today + " " + dt_string.split(" ")[0] + " and it is " + new_ora + ".\nHow can I help you?"
        dispatcher.utter_message(message.replace("\n", "  \n"))
        return [Restarted()]
...
class AprireSegnalazioneTestoForm(FormAction):
    ...
    def validate_aprire_segnalazione_testo(self, value: Text, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any], ) -> Dict[Text, Any]:
        ...
        if len(str(value).strip()) != 0:
            ...
            dispatcher.utter_message("The message of the report cannot be empty.\nRepeat the entry.")
...
class ActionSalvaSegnalazione(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if segnalazione.lower().strip() != "exit":
            ...
            conn.commit()
            dispatcher.utter_message("Your report has been completed, you will be contacted as soon as possible.")
        else:
            dispatcher.utter_message("Report cancelled.")
        conn.close()
...
class TestoForm(FormAction):
    ...
    def validate_testo(self, value: Text, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any], ) -> Dict[Text, Any]:
        ...
        if len(str(value).strip()) != 0:
            ...
            dispatcher.utter_message("The message to be sent to the operator cannot be empty.  \nRepeat the entry.")
            return {"testo": None}
...
class ActionOperatore(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        db_connection_last_interaction()
        dispatcher.utter_message("You will be contacted soon by an operator (at any time you can end the chat by typing 'exit').  \n  \nWrite now the message to sent to one of our operators and wait for his feedback, thanks.")
...
class ActionInsertEmail(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if assertion != "no":
            if email == "":
                message = "Glad to have been helpful to you! Please enter an email address in order to improve this service.\n"
            else:
                message = "Glad to have been helpful to you!\n"
        else:
            if email == "":
                message = "Please enter an email address to be contacted as soon as possible.\n"
            else:
                message = "You will be contacted as soon as possible at the previously entered email address.\n"
        dispatcher.utter_message(message.replace("\n", "  \n"))
...
class ActionEmailOperatore(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if email == "":
            message = "In a moment you can text to an operator.\nLeave your email to be contacted in case there are no operators available.\n"
            dispatcher.utter_message(message.replace("\n", "  \n"))
...
class ActionAprireSegnalazioneEmail(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if email == "":
            message = "Enter an email address to open the report.\n"
        else:
            message = "The email address entered previously will be used to open the report.\n"
        dispatcher.utter_message(message.replace("\n", "  \n"))
        return
